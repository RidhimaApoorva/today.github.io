...
?>
 ...
 //dis cde for display the image
	...
	while($row=mysql_fetch_array($query))
	{
		...
		echo "	
		<tr>
			<td><img src='image/$image1' max-height='30%' width='50%'></img></td>
			<td><img src='image/$image2' max-height='30%' width='50%'></img></td>
			<td><img src='image/$image3' max-height='30%' width='30%'></img></td>
			<td>$cname</td>
...
if(isset($_POST['send']))
{
	...
	$sql="insert into `service`(cname,image1,image2,image3)values('$cname','$image1','$image2','$image3')";
	if(mysql_query($sql))
	{
		echo "<script>alert('Done')</script>";
		echo "<script>window.open('serve_user.php','_self')</script>";// no need to refresh page to display inserted item
	}
	else
	{
		echo "<script>alert('Error')</script>";
		echo "<script>window.open('admin_serve.php','_self')</script>";
	} 
}
